...
for(class.size in c("equal","Sunequal","unequal")) {
  for(class.sep in c("low","medium","high")){
    for(n in 1:numsim){
      ...
      outputB=paste("SimulationStudy/Results/Output/BCHoutMan/","out-",data.cond,paste(".txt"),paste("'"),paste(";"), sep="")
      
      inputfile=paste("/Users/christinakamis/Documents/DukeSociology/Dissertation/SimulationStudy/Results/InputFiles/BCHinpMan/","inp-",data.cond,paste(".inp"), sep="")
      
      
      input=file(inputfile) 
      writeLines(c(
        paste("TITLE:'BCH-manual';"),
        paste("DATA:")  ,
